...
void calc_uniform_mean_curvature(Mesh& mesh, std::vector< Vec3 >& laplacienOut){
    // vunicurvature_ =  la courbure moyenne approximation en utilisant l'opérateur uniforme de Laplace mesh.vertices.size()
    std::vector<std::vector<unsigned int> > oneRing;
    collect_one_ring(mesh.vertices, mesh.triangles, oneRing);

    laplacienOut.clear();
    laplacienOut.resize(mesh.vertices.size());

    for(int i {0}; i < mesh.vertices.size(); ++i){
        std::vector<Vec3> voisins;
        //std::cout << "Point courant : " << mesh.vertices[i][0] << mesh.vertices[i][1]<< mesh.vertices[i][2] << '\n';
        for(int j{0}; j < oneRing[i].size(); ++j){
            voisins.push_back(mesh.vertices[oneRing[i][j]]);
            //std::cout << "voisin n" << j << " : "<< voisins[j][0] << voisins[j][1] << voisins[j][2] << '\n';
        }
        Vec3 centroidV { centroid(voisins) };

        laplacienOut[i] = (centroidV - mesh.vertices[i]);
    }
}

void calcTriangleQuality(Mesh& mesh){ // mesh.triangles.size()
    for(int i {0}; i < mesh.triangles.size() ; ++i){

        std::array<Vec3, 3> vertices;
        for(int j {0}; j < 3; ++j){
            vertices[j] =  mesh.vertices[mesh.triangles[i].v[j]];
        }

        Vec3 e1 { vertices[1] - vertices[0]};
        Vec3 e2 { vertices[2] - vertices[0]};

        float denom = Vec3::cross(e1, e2).length();
        if( denom < 0.001 ){
            mesh.triangles[i].quality = 100.0f;
        }else {
            mesh.triangles[i].quality = denom / 2.0f;
        } 
...
void calc_mean_curvature(Mesh& mesh, std::vector<Vec3>& laplacienOut ){
    std::vector<std::vector<unsigned int> > oneRing;
    ...
    for(int i {0}; i < mesh.vertices.size(); ++i){
        ...
        for(int j{0}; j < voisins.size(); ++j){ // Laplace-Beltrami
            Vec3 v = voisins[j] -  mesh.vertices[i];
            Vec3 vPrev;
            Vec3 vSuiv;
            if(j == 0) vPrev = voisins[voisins.size()-1];
            else vPrev = voisins[j-1];

            if(j == voisins.size()-1) vSuiv = voisins[0];
            else vSuiv = voisins[j+1];
            
            float angleA = acos(Vec3::dot(vPrev, v) / (vPrev.length() * v.length()));
            float angleB = acos(Vec3::dot(vSuiv, v) / (vSuiv.length() * v.length()));
            weightSum += angleA + angleB;
            laplacienOut[i] += (angleA + angleB) * v;
        }
        laplacienOut[i] /= weightSum;
    }
...
void calc_gauss_curvature(Mesh& mesh, std::vector<float>& field ){
    ...
    field.clear();
    field.resize(mesh.vertices.size());
    ...
    for(int i {0}; i < mesh.vertices.size(); ++i){
        ...
        for(int j{0}; j < voisins.size(); ++j){ // Laplace-Beltrami
            ...
            Vec3 v2 =   vSuiv -  mesh.vertices[i];
            
            angleSum += angle(v, v2);
            
        }
        float gauss = 2 * M_PI * angleSum;
        field[i] = gauss;
    }
...
void uniform_smooth(Mesh& mesh, unsigned int _iters){
    std::vector<Vec3> laplacien;
    for(int i {0}; i < _iters; ++i){
        calc_uniform_mean_curvature( mesh, laplacien);
...
void beltramiSmooth(Mesh& mesh, unsigned int _iters){
    std::vector<Vec3> laplacien;
    for(int i {0}; i < _iters; ++i){
        calc_mean_curvature( mesh, laplacien);
        for(int j {0}; j < mesh.vertices.size(); ++j){
            mesh.vertices[j] += 0.5 * laplacien[j];
        }
    }
...
void addNoise(Mesh& mesh){
    ...
    for(int i {0}; i < mesh.vertices.size(); ++i){
        ...
        mesh.vertices[i] = Vec3( p[0] + factor*((double)(rand()) / (double)
...
int main (int argc, char ** argv) {
    ...
    mesh.computeNormals();
    mesh.offset(Vec3{-1.0,0.0f, 0.0f});
    addNoise(mesh);

    openOFF("data/avion_n.off", mesh2.vertices, mesh2.normals, mesh2.triangles, mesh2.triangle_normals);
    mesh2.computeNormals();
    mesh2.offset(Vec3{0.0,0.0f, 0.0f});
    mesh2.computeNormals();
    addNoise(mesh2);

    openOFF("data/avion_n.off", mesh3.vertices, mesh3.normals, mesh3.triangles, mesh3.triangle_normals);
    mesh3.computeNormals();
    mesh3.offset(Vec3{1.0,0.0f, 0.0f});
    mesh3.computeNormals();
    addNoise(mesh3);

    //calcTriangleQuality(mesh);

    uniform_smooth(mesh2, 20);

    taubinSmooth(mesh3, 20, 0.3, -0.3);

    beltramiSmooth(mesh3, 4); 



    // A faire : normaliser les champs pour avoir une valeur flotante entre 0. et 1. dans current_field
    //***********************************************//
    calc_gauss_curvature(mesh, current_field);
    /*
    float sum{};
    for(const auto& f : current_field){
        sum += f;
    }
    float invSum{1/sum};

    for(auto& f : current_field){
        f *= invSum;
    }
    */
