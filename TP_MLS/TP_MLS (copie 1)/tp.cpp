    ...
    void HPSS(const Vec3& inputPoint, Vec3& outputPoint, Vec3& outputNormal, 
        ...
        for(size_t i{0}; i < nbIterations; ++i){
            ...
            for(unsigned int i{0} ; i < knn; i++){
                ...
                if(r2 < square_distances_to_neighbors[i]  ) continue; // si c'est plus loins que notre radius on skip
...
void dualContouring(const std::vector<Vec3>& positions, int gridSize, 
    ...
    Array3D<Vec3> vec3Grid(gridSize-1, std::vector<std::vector<Vec3>>(gridSize-1, std::vector<Vec3>(gridSize-1, Vec3(0.0f, 0.0f, 0.0f))));
        ...
        for (int j{0}; j < gridSize-1; j++){
            for (int k{0}; k < gridSize-1; k++){
                ...
                if(isNeg && isPos) {                    
                    ...
                    if (centerSdf >= 0.f) dbgCentersPos.push_back(center);
                    else dbgCentersNeg.push_back(center);
                }
            }
        }
    }

    // reparcourir pour connecter les points
...
int main (int argc, char ** argv) {
...

    {
        ...
        dualContouring(positions, /*gridSize*/63, kdtree, KernelType::GAUSSIEN, /*radius*/1.0f);
